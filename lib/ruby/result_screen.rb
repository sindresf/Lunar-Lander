...
class ResultScreen
  include Screen
  def initialize(game, menu_screen, world, multiplayer, muted, scores)
    @game = game
  ...
  def render(gdx_delta)
    ...
    @font.draw(@batch, "Score 1: #{@scores[0].round(1)}", 200, 300);
    if @multiplayer
      @font.draw(@batch, "Score 2: #{@scores[1].round(1)}", 200, 340);
    end
